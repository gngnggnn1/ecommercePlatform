    ...
    <template>
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="success" size="mini" @click="handleOn(scope.$index, scope.row)" :disabled="getTag(scope.row.sts_cd)[1]=='已上架'?true:false">上架</el-button>
          <el-button type="danger" size="mini" @click="handleOff(scope.$index, scope.row)"  :disabled="getTag(scope.row.sts_cd)[1]=='未上架'?true:false">下架</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="block">
          <el-row :gutter="100">
            ...
            <el-col :span="8" class="toolbar" style="padding-bottom: 0;">
              <el-button
                    type="primary"
                    icon="el-icon-import"
                    v-on:click="batchOn(selectedItem)"
                  >批量上架</el-button>
                  <el-button
                    type="primary"
                    icon="el-icon-import"
                    v-on:click="batchOff(selectedItem)"
                  >批量下架</el-button>
            </el-col>
            <el-col :span="8">
              <!--页面分割-->
...
export default {
  ...
  methods: {
    ...
    handleOff(index, row) {
      let m = {pro_id: row.pro_id, sts_cd: "B"}
      changeStatus(m).then(res => {
        if(res.code == 200){
          alert("商品状态更新成功")
        }
      ...
      this.$confirm("确认操作?", "批量上架", {
        ...
        .then(() => {
          rows.forEach(rowElement => {
            let m = {pro_id: rowElement.pro_id, sts_cd: "B"}
            changeStatus(m).then(res => {
              console.log(res)
            })
          });
