      ...
      <div class="block">
        <el-row :gutter="100">
          ...
          <el-col :span="8" class="toolbar" style="padding-bottom: 0;">
            ...
            <el-button type="primary" icon="el-icon-import" v-on:click="refresh">刷新</el-button>
          </el-col>
          <el-col :span="8">
            <!--页面分割-->
            <el-pagination
    ...
    <el-dialog title="商品信息" :visible.sync="dialogVisible" width="50%">
      <el-form ref="form" :model="form" label-width="80px">
        <el-form-item label="商品ID">
          <el-input  v-model="form.pro_id"  style="width: 100px;" disabled>
          </el-input>
        </el-form-item>
        <div style="color: orange; margin-left: 13px; margin-bottom: 10px">商品标题准确、简洁、完整</div>
        <el-form-item label="商品标题">
          <el-input  v-model="form.title" placeholder="商品标题" style="width: 180px;">
          </el-input>
        </el-form-item>
        ...
        <el-form-item label="上传图片" v-model="form.uris">
          <el-upload
        class="upload-demo"
        ref="upload"
...
<script>
import { searchUnionInfo, deletePro, changeStatus, getAllOptions, updateGoodsMain} from "@/api/mvoHandler";
...
searchUnionInfo(this.curUser.username).then(res => {
    ...
    handleSubmit(){
      updateGoodsMain(this.form).then(res => {
        console.log(res)
        if(res.data == 1){
          alert("更新成功！")
          searchUnionInfo(this.curUser.username).then(res => {
    ...
    handleSuccessUpload(res, file){
      this.uriList.push("http://localhost:8088/static/upload/" + res)
      this.form.uris = this.uriList
      console.log(this.form.uris)
      alert("上传成功")
    },
    handleErrorUpload(){
      alert("图片上传失败")
