  ...
  <div class="pageContent">
    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <span>账户信息</span>
        <el-button style="float: right; padding: 3px 0; margin-left: 20px" type="text">提现</el-button>
    ...
    <el-dialog title="提现" :visible.sync="withdrawVisible" @click="this.withdrawVisible = true" width="50%">
      <el-form ref="withdrawForm" :model="withdrawForm" label-width="80px">
        ...
        <el-form-item label="支付密码">
          <el-input v-model="withdrawForm.password"></el-input>
        </el-form-item>
      </el-form>
...
<script>
import { getWalletInfo, searchBusinessFlow} from "@/api/wallet";
export default {
  data() {
    return {
      username:'',
  ...
  methods: {
    ...
    handleWithDraw(){
      ...
      waa_['username'] = this.username
      let waf_ = {}
      waf_ = this.form1
      waf_['available_money'] = this.form1.available_money - this.withdrawForm.available_money
      waf_['withdrawing_money'] = this.withdrawForm.withdrawing_money
      waf_['last_update_time'] = new Date()
      let wta_ = {}
      ...
      wta_['available_money_after'] = this.form1.available_money - this.withdrawForm.withdrawing_money
      wta_['depositing_money_after'] = this.form1.depositing_money
      wta_['withdrawing_money_after'] = this.form1.withdrawing_money + this.withdrawForm.withdrawing_money
      wta_['status'] = 2
      wta_['create_time'] = new Date()
      wta_['last_update_time'] = new Date()
      let wtr_ = {}
      wtr_['buyer_id'] = this.buyer_id
      wtr_['account_name'] = this.form.account_name
      wtr_['balance'] = this.form1.available_money - this.withdrawForm.withdrawing_money
      wtr_['transaction_type'] = 2
      wtr_['transaction_money'] = this.withdrawForm.withdrawing_money
      wtr_['finance_type'] = 2
      wtr_['last_update'] = new Date()
      request['waa'] = waa_
      request['waf'] = waf_
      request['wta'] = wta_
      request['wtr'] = wtr_
