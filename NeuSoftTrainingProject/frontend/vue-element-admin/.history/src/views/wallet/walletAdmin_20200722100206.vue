    ...
    export default {
        ...
        methods:{
          getInfo(){
            this.$store.dispatch('GetInfo').then((result) => {
              ...
              for(let i =0; i< result.data.wtr.length; i++){
                let f = result.data.wtr[i]
                if(result.data.wtr[i].status == '2'){
                 f.status = '申请中'
                }
                if (result.data.wtr[i].status == '4'){
                  f.status = '已完成'
                }
                if (result.data.wtr[i].transaction_type == '1'){
                  f.transaction_type = '充值'
           
                }
                if (result.data.wtr[i].transaction_type == '2'){
                  f.transaction_type = '提现'
                }
          ...
          shipConfirm(index, rowData){
            ...
            if(f.transaction_type == '充值'){
              f.transaction_type = 1
            }
            if(f.transaction_type == '提现'){
              f.transaction_type = 2
            }
          ...
          batchShip(rows){
            ...
            if(f.transaction_type == '提现'){
              ...
              this.$store.dispatch('ShipConfirm',f).then((result) => {
